...
## Article-ready output layer

The final manuscript-facing outputs are generated by `scripts/22_create_article_ready_sqi_outputs.py`. This script does not redefine SQI candidates or alter the analytical pipeline. It reads the existing validation outputs and produces clean article-ready tables and figures using reader-facing names:
...
The script also produces an ECe sensitivity table for methodological documentation. Internal candidate codes are retained in the analysis pipeline but are not used in manuscript-facing figures or tables.
